...
//////////////////////////////////////////////////////
int createSocket(int timeoutSecs)
{
    ...
    if (dgram_socket > 0)
    {
        struct timeval timeout = { timeoutSecs, 0 }; //set timeout for 'x' seconds
        setsockopt(dgram_socket, SOL_SOCKET, SO_RCVTIMEO, (char*)&timeout, sizeof(struct timeval));
    }
    return dgram_socket;
}
...
void validateReturnCode(int rc, char* msg, int socket_desc)
{
    ...
    {
        if (!lastRecvTimedOut(rc))        // do not display error in case of timeouts
            perror(msg);
    }
}

/////////////////////////////////////////////////////////////////////////
/// This method is used to check if last recvFrom call timed out or not.
///     Returns 1 in case of timeout else 0
/////////////////////////////////////////////////////////////////////////
int lastRecvTimedOut(int numBytesRecvd)
{
    return ((errno == EAGAIN || errno == EWOULDBLOCK) && numBytesRecvd < 0) ? 1 : 0;
}
...
struct recvd_packet_info getRemotePlayerRequest(int socket_desc)
{
    ...
    if (!lastRecvTimedOut(rc))
    {
        printSendRecvMsg("RECVD_MSG (%d bytes): Remote player (%s) --> '%03d:%03d:%03d:%03d:%03d:%03d'", rc, recvdRequest.addr_str,
            recvdRequest.recvdMsg.version, recvdRequest.recvdMsg.cmdCode, recvdRequest.recvdMsg.responseCode,
            recvdRequest.recvdMsg.move, recvdRequest.recvdMsg.turnNumber, recvdRequest.recvdMsg.gameId);
    }
