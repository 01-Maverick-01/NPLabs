...
/********************************** #define section *************************************/
#define MAX_VERSION 3                                                   // maximum supported game version
#define MIN_VERSION_WITH_RETRY_SUPPORT 3                                // minimum version with support for retry
#define MIN_VERSION 1                                                   // minimum supported game version
#define MAX_GAME 3                                                      // supports a maximum of 256 games 
#define TIMEOUT 5                                                       // socket timeout for recvFrom
#define MAX_RETRY 5                                                     // maximum number of retries
#define INACTIVE_WAIT_MULTIPLIER 4                                      // multiplier to wait after final msg sent to end game
...
struct GameData
{
    ...
    time_t lastSendTime;
    int numRetries;
    struct msg lastSentMsg;
    struct msg lastRecvdMsg;
...
void                resendMsgsToConnectedPlayers(struct ServerData *serverData);
void                updateLastSentMsg(struct GameData *gameData, int version, int move, int cmd, int turn, int returnCode, int gameId);
void                sendMsgToRemote(struct GameData *gameData, struct msg msg, int sock_desc, struct sockaddr_in remoteAddr, int resetNumRetries);
void                resendLastMoveToRemote(struct GameData *gameData, int socket_desc);
void                updateLastRecvdMsg(struct GameData *gameData, struct msg recvdMsg);
void                markForEnd(struct GameData *gameData, struct sockaddr_in remoteAddr, char *reason);
...
int main(int argc, char *argv[])
{
    ...
    data.socket = createSocket(TIMEOUT);
    if (data.socket < 0)
        exit(EXIT_FAILURE);
...
struct ServerData validateInput(int argc, char** argv)
{
    ...
    for (int i = 0; i < MAX_GAME; i++)
    {
        ...
        serverData.allGames[i].numRetries = 0;
    }
...
void handlePlayerRequest(struct recvd_packet_info playerRequest, struct ServerData *serverData)
{
    if (lastRecvTimedOut(playerRequest.numBytesRecvd))
        resendMsgsToConnectedPlayers(serverData);
    else
    {
        if (validateVersionResCodeAndMsgSize(playerRequest, serverData))
        {
            switch(playerRequest.recvdMsg.cmdCode)
            {
                ...
                default:
                    ...
                    sendInvalidRequestResCode(serverData->socket, playerRequest, NULL);
                    break;
            }
        }
        else
            markForEnd(&serverData->allGames[playerRequest.recvdMsg.gameId], playerRequest.remoteAddr, "");
    }
}
...
void startNewGame(struct recvd_packet_info playerRequest, struct ServerData *serverData)
{
    ...
    if (nextFreeSlot >= 0)
    {
        ...
        updateLastRecvdMsg(gameData, playerRequest.recvdMsg);
        ...
        if (updateBoard(move, serverData, nextFreeSlot, serverData->mark))
        {
            ...
            sendMsgToRemote(gameData, msg, serverData->socket, playerRequest.remoteAddr, 1);
        }
    }
    else if (nextFreeSlot == -2)
    {
        printGameErr("Remote player's (%s) game request declined as no free slot available", playerRequest.addr_str);
        sendNotOKReturnCode(serverData->socket, 0, RES_CODE_SERVER_BUSY, playerRequest, NULL);
    }
}
...
void continueExistingGame(struct recvd_packet_info playerRequest, struct ServerData *serverData)
{
    if (!isAlreadyConnected(playerRequest, *serverData))
    {
        ...
        sendNotOKReturnCode(serverData->socket, 0, RES_CODE_GAME_ID_MISMATCH, playerRequest, NULL);
        return;
    }
    ...
    if (playerRequest.recvdMsg.responseCode == RES_CODE_GAME_OVER_ACK)
    {
        if (gameData->state != WaitingForAck)
        {
            ...
            sendInvalidRequestResCode(serverData->socket, playerRequest, gameData);
        }
    ...
    if (gameData->turn - 1 == playerRequest.recvdMsg.turnNumber)                            // if duplicate packet received
    {
        if (playerRequest.recvdMsg.move == gameData->lastRecvdMsg.move)                     // resend last packet if same move recvd
            resendLastMoveToRemote(gameData, serverData->socket);
        else
            sendInvalidRequestResCode(serverData->socket, playerRequest, gameData);         // otherwise, send error
        return;
    }
    ...
    if (gameData->turn + 1 != playerRequest.recvdMsg.turnNumber)
    {
        ...
        sendNotOKReturnCode(serverData->socket, gameId, RES_CODE_NO_SYNC, playerRequest, gameData);
        return;
    }
    
    if (gameData->state == Inactive)
    {
        printGameMsg("Changed remote player's (%s) game state from inactive to active.", playerRequest.addr_str);
        gameData->state = Started;
    }
    ...
    if (updateBoard(playerRequest.recvdMsg.move, serverData, gameData->id, 'O'))
    {
        ...
        if (checkWin == 0)
        {
            ...
            if (checkWin == 1 || checkWin == 2)
            {
                ...
                sendMsgToRemote(gameData, msg, serverData->socket, gameData->clientAddr, 1);
        ...
        else
        {
            ...
            gameData->turn++;
            sendGameCompleteAck(serverData->socket, gameId, playerRequest, gameData);
            handleGameOver(checkWin, playerRequest.addr_str, 1, serverData->mark, gameData->board);
            markForEnd(gameData, playerRequest.remoteAddr, "Game moved to fin wait period.");
        }
    }
    else
    {
        sendInvalidMoveResCode(serverData->socket, gameId, playerRequest, gameData);
    }
}
...
int isAlreadyConnected(struct recvd_packet_info playerRequest, struct ServerData data)
{
    ...
    else
    {
        ...
        if (gameData->state == Inactive && difftime(time(NULL), gameData->lastSendTime) >= TIMEOUT * INACTIVE_WAIT_MULTIPLIER)
        {
            endGame(gameData);
            return 0;
        }
...
int getNextFreeGameSlot(struct ServerData serverData, struct recvd_packet_info playerRequest)
{
    ...
    if (!isAlreadyConnected(playerRequest, serverData))
    {
        for (int i = 0; i < MAX_GAME; i++)
        {
            ...
            else if (gameData->state == Inactive)
                numInactive++;
            else if (gameData->id == -1)
                return i;
            else
                numBusy++;            
        }
        printError("Total number of sessions = %d, in use = %d, in inactive state = %d", MAX_GAME, numBusy, numInactive);
        return -2;
    }
    else
    {
...
int validateVersionResCodeAndMsgSize(struct recvd_packet_info playerRequest, struct ServerData *serverData)
{
    ...
    if (playerRequest.recvdMsg.version < MIN_VERSION || playerRequest.recvdMsg.version > MAX_VERSION)
    {
        ...
        sendNotOKReturnCode(serverData->socket, playerRequest.recvdMsg.gameId, RES_CODE_VERSION_MISMATCH, playerRequest, NULL);
        return 0;
    }
...
void sendNotOKReturnCode(int sock_desc, int gameId, int responseCode, struct recvd_packet_info playerRequest, struct GameData *gameData)
{
    if (gameData != NULL)
    {
        ...
        sendMsgToRemote(gameData, msg, sock_desc, playerRequest.remoteAddr, 1);
        markForEnd(gameData, playerRequest.remoteAddr, "Game moved to fin wait period.");
    }
    else
...
void endGame(struct GameData *gameData)
{
    printGameMsg("Game cleaned and session reclaimed as it was no longer needed.");
    gameData->id = -1;
    gameData->turn = -1;
...
//////////////////////////////////////////////////////
/// This method is to move a game to inactive state.
//////////////////////////////////////////////////////
void markForEnd(struct GameData *gameData, struct sockaddr_in remoteAddr, char *reason)
{
    if (isSameAddr(remoteAddr, gameData->clientAddr))
    {
        gameData->state = Inactive;
        if (strlen(reason) > 0)
            printGameMsg("Game session with id = '%d' moved to inactive state. Reason: %s", gameData->id, reason);
    }
}

//////////////////////////////////////////////////////////////////////////////////////////////////
/// This method resends lastSentMsg to all connected remote players for which last recv timed out
//////////////////////////////////////////////////////////////////////////////////////////////////
void resendMsgsToConnectedPlayers(struct ServerData *serverData)
{
    ...
    {
        struct GameData *gameData = &serverData->allGames[i];
        if ((gameData->state == Started || gameData->state == WaitingForAck) && difftime(currentTime, gameData->lastSendTime) >= TIMEOUT)
        {
            if (gameData->numRetries == MAX_RETRY)
                markForEnd(gameData, gameData->clientAddr, "Game timed out.");
            else
            {
                if (gameData->version >= MIN_VERSION_WITH_RETRY_SUPPORT)
                    resendLastMoveToRemote(gameData, serverData->socket);
                gameData->numRetries++;
            }
        }
...
//////////////////////////////////////////////////////////
/// Call this method to resend last move to remote player
//////////////////////////////////////////////////////////
void resendLastMoveToRemote(struct GameData *gameData, int socket_desc)
{
    printGameMsg("Resending move to client with game session id = '%d'", gameData->id);
    sendMsgToRemote(gameData, gameData->lastSentMsg, socket_desc, gameData->clientAddr, 0);
}
...
void sendMsgToRemote(struct GameData *gameData, struct msg msg, int sock_desc, struct sockaddr_in remoteAddr, int resetNumRetries)
{
    ...
    updateLastSentMsg(gameData, version, move, cmdCode, turnNumber, responseCode, gameId);

    if (resetNumRetries)
        gameData->numRetries = 0;
}
...
void updateLastSentMsg(struct GameData *gameData, int version, int move, int cmd, int turn, int returnCode, int gameId)
{
    ...
    gameData->lastSentMsg.version = version;
    gameData->lastSentMsg.cmdCode = cmd;
    gameData->lastSentMsg.responseCode = returnCode;
    gameData->lastSentMsg.move = move;
    gameData->lastSentMsg.turnNumber = turn;
    gameData->lastSentMsg.gameId = gameId;
}

void updateLastRecvdMsg(struct GameData *gameData, struct msg recvdMsg)
{
    memcpy(&gameData->lastRecvdMsg, &recvdMsg, sizeof(struct msg));
}